...
function lib:getImage(path, force)
	...
	-- An ALREADY-LOADED image, on an engine whose Image is a table.
	--
	-- Desktop LOVE returns userdata from love.graphics.newImage, so the
	-- check above is the whole test there. wasmcart-lua returns a table
	-- ({id=, w=, h=} with an Image metatable), which falls through to the
	-- string/task branch below, finds path.task nil, and quietly returns
	-- nothing -- so every material with a hand-made texture samples the
	-- DEFAULT white texture instead. The symptom is a texture that is
	-- correctly created, correctly assigned, and simply never appears:
	-- a solid red texture still renders a pure white ball.
	if type(path) == "table" and (path.id or path.w) and not path.task then
		return path
	end
	if not path then
		return false
