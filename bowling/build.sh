...
# assets.index MUST be regenerated before packing, and this has to come
# AFTER $WASMCART_LUA resolves -- putting it earlier silently does nothing,
# and 3DreamEngine then fails at boot because it discovers its own modules
# by listing directories, which a cart's asset bundle cannot do.
"$WASMCART_LUA/tools/gen-asset-index.sh" "$HERE/app" > /dev/null

cp "$ENGINE" "$HERE/main.wasm"
node "$WASMCART_PACK" \
  --wasm "$HERE/main.wasm" --assets "$HERE/app" \
  --name "Bowling" --width 1920 --height 1080 \
  -o "$HERE/bowling.wasc" > /dev/null
echo "packed $(du -h "$HERE/bowling.wasc" | cut -f1)"
