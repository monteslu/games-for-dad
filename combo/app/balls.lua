...
-- SEVEN MERGE TIERS, not a pool rack. Two balls of the same tier merge into
-- one of the next tier up; merging the top tier detonates.
--
-- The ramp is NuSan's from Combo Pool (PICO-8 indices 1,13,6,9,10,8,14),
-- converted to RGB. It runs dark -> pale -> hot, which reads as a
-- progression at a glance rather than as seven arbitrary colours, and the
-- hottest ball being the rarest is the right instinct.
M.TIERS = 7
M.COLORS = {
  [1] = {0.11, 0.17, 0.33},   -- dark blue
  [2] = {0.51, 0.46, 0.61},   -- lavender
  [3] = {0.76, 0.76, 0.78},   -- light grey
  [4] = {1.00, 0.64, 0.00},   -- orange
  [5] = {1.00, 0.93, 0.15},   -- yellow
  [6] = {1.00, 0.00, 0.30},   -- red
  [7] = {1.00, 0.47, 0.66},   -- pink, the detonator
}

-- Score is what a tier is WORTH; cost is what it drains from the life bar
-- while it sits on the table. They are deliberately INVERTED: a low ball is
-- worth almost nothing and costs the most to keep, so merging up is
-- simultaneously how you score and how you survive. One number pair does
-- both jobs and the player never has to be told the strategy.
M.VALUE = { 1, 2, 3, 5, 10, 20, 100 }
M.COST  = { 4, 3.5, 3, 2, 1.5, 1, 0 }
...
function M.makeFaces()
  ...
  for n = 1, M.TIERS do
    ...
    -- Solid tier colour. No stripes: the tier IS the colour, and a stripe
    -- would fight the one signal the player has to read instantly.
